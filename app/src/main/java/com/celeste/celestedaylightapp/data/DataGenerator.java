...
//    }
     ...
     * @return list of object
     */
    public static List<User> getPeopleData(Context ctx) {
        ...
        String emails_arr[] = ctx.getResources().getStringArray(R.array.people_emails);

        for (int i = 0; i < drw_arr.length(); i++) {
            User obj = new User();
            obj.setFirstName(name_arr[i]);
            obj.setUserEmail(emails_arr[i]);
            obj.setUserProfileImage(drw_arr.getResourceId(i, -1));
          //  obj.setUserEmail(Tools.(obj.getUserEmail()));
            obj.imageDrw = ctx.getResources().getDrawable(obj.getUserProfileImage());
            items.add(obj);
        }
      //  Collections.shuffle(items);
        return items;
